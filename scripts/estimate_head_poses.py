...
def main(args):
    ...
    videos_progress = tqdm.tqdm(
        videos_paths, total=len(videos_paths), desc="head pose")
    for video_path in videos_progress:
        ...
        if not os.path.exists(frames_root):
            continue
        #
        faces_detection_file_path = os.path.normpath(os.path.join(
            dataset_root, "faces", args.dataset, video_name, 'faceinfo.pkl'))
        # output
        faces_detection_with_pose_file_path = os.path.normpath(os.path.join(
            dataset_root, "faces", args.dataset, video_name, output_file_name))

        if not os.path.exists(faces_detection_file_path):
            print(
                f"faces detection file {faces_detection_file_path} not found")
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', action='store_true',
                        help='if true existed frames of an existed participant will not be replaced')
    #
