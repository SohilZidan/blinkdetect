...
if __name__=="__main__":
    ...
    for video_path in tqdm.tqdm(videos_paths, total=len(videos_paths), desc="video"):
        ...
        # input
        input_file_path_hdf5 = os.path.normpath(os.path.join(dataset_root,"eye_landmarks_v2", args.dataset, video_name,"eyeinfo.hdf5"))
        if not os.path.exists(input_file_path_hdf5):
            print(f"{input_file_path_hdf5} not existed")
            continue
    
        # output
        output_path = os.path.normpath(os.path.join(dataset_root,"tracked_faces_v2", args.dataset, video_name, "timeseries_plots"))
        output_results_path = os.path.normpath(os.path.join(dataset_root,"tracked_faces_v2", args.dataset, video_name, "signals"))
        # results files paths
        ...
        pupil2corner_file_path = os.path.join(output_results_path, "pupil2corner.pkl")
        ...
        max_pupil2corner_right = find_max(_frames['right_pupil2corner'].tolist())
        ...
        for idx, _frame in tqdm.tqdm(enumerate(ordered_frames), total=len(ordered_frames), desc="frame"):
            ...
            _means["right"].append([0, 0,0] if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["right_mean_color"])
            _means["left"].append([0, 0,0] if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["left_mean_color"])
            
            _stds["right"].append([0, 0,0] if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["right_std_color"])
            _stds["left"].append([0, 0,0] if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["left_std_color"])

            _eyelids_dists["right"].append(0 if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["right_eyelids_dist"])
            _eyelids_dists["left"].append(0 if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["left_eyelids_dist"])

            _diameters["right"].append(0 if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["right_diameter"])
            _diameters["left"].append(0 if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["left_diameter"])

            _pupil2corners["right"].append(0 if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["right_pupil2corner"])
            _pupil2corners["left"].append(0 if _frames.loc[_frame]["faces_not_found"]==1 else _frames.loc[_frame]["left_pupil2corner"])
            ...
            if _frames.loc[_frame]["faces_not_found"]==1:
                ...
                _eyelids_dists["best"].append(_frames.loc[_frame]["right_eyelids_dist"])
                _diameters["best"].append(_frames.loc[_frame]["right_diameter"])
                _pupil2corners["best"].append(_frames.loc[_frame]["right_pupil2corner"])
            else:
                _means["best"].append(_frames.loc[_frame]["left_mean_color"])
                _stds["best"].append(_frames.loc[_frame]["left_std_color"])
                _eyelids_dists["best"].append(_frames.loc[_frame]["left_eyelids_dist"])
                _diameters["best"].append(_frames.loc[_frame]["left_diameter"])
                _pupil2corners["best"].append(_frames.loc[_frame]["left_pupil2corner"])
