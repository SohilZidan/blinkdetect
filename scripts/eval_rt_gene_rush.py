...
def evaluate(prd_blinking_annotation, gt_blinking_annotation):
    ...
    fmeasure.evaluate({"Blinking":gt_blinking_annotation},
...
if __name__=="__main__":
    ...
    results = {}
    vid_progress = tqdm.tqdm(list(zip(preds_paths, frames_paths)), total=len(preds_paths), desc="participants")
    for preds_path, frames_path in vid_progress:
        ...
        for _frame_d in range(int(frames[0]), int(frames[-1])):
            ...
            if _frame not in frames:
                closed_eyes = closed_eyes[:_frame_d-1] + [[False]] + closed_eyes[_frame_d-1:]
        #
        ...
        results[video_name] = {"metric": evaluate(blink_preds, blinking_anns)}
