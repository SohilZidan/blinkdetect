...
iris_net.load_weights("irislandmarks.pth")
...
dataset = os.path.normpath(os.path.join(dataset_root, "RN"))
...
if __name__=="__main__":
    ...
    for video_path in videos_progress:
        ...
        faces_detection_file_path = os.path.normpath(os.path.join(dataset_root,"faces", "RN", video_name, 'faceinfo_v2.pkl'))
         # load detections
        with open(faces_detection_file_path, "rb") as _dets_file:
            faces_detections = pickle.load(_dets_file)
        # output
        new_dir = os.path.join(dataset_root,"eye-cutouts", "RN", video_name)
        ...
        for frame in tqdm.tqdm(frames, total=len(frames)):
            ...
            down = int(bbox_org[3]+0.25*H) if int(bbox_org[3]+0.25*H) < img.shape[0] else img.shape[0]
            left = int(bbox_org[0]-0.25*W) if int(bbox_org[0]-0.25*W) > 0 else 0
            right = int(bbox_org[2]+0.25*W) if int(bbox_org[2]+0.25*W) < img.shape[1] else img.shape[1]
            bbox_m = [left, up, right, down]
            facial_img_m = img[bbox_m[1]:bbox_m[3], bbox_m[0]:bbox_m[2]]
            # alignment
            aligned_facial_img = alignment_procedure(facial_img_m, right_eye, left_eye)
            # face mesh
            ...
            detections_left = np.array(list(map(detections.__getitem__, left_eye)))
            detections_right = np.array(list(map(detections.__getitem__, right_eye)))
            ...
            for eye_key in _detections.keys():
                ...
                X, Y = np.min(eye.reshape(-1,3)[:,:2], axis=0).astype(np.int)
                X1,Y1 = np.max(eye.reshape(-1,3)[:,:2], axis=0).astype(np.int)
                eye_region = eye_region[ Y:Y1,X:X1,:]
                #
                eye_region = cv2.cvtColor(eye_region, cv2.COLOR_RGB2BGR)
                eye_path = os.path.join(new_dir,eye_key)
                if not os.path.exists(eye_path): os.makedirs(eye_path)
