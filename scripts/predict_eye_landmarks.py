...
from bdlib.metrics.distance import iris_diameter, eyelids_directed_hausdorff
from bdlib.image.analyze import color_analysis
from bdlib.image.misc import cut_region, expand_region
from bdlib.core import points_in_between
...
def overlay_image(aligned_facial_img: np.ndarray, resp):
    ...
    for eye_key in resp:
        ...
        for i in range(1, 16):
            ...
            aligned_facial_img_eyelids = cv2.circle(aligned_facial_img_eyelids, (
                center[0], center[1]), radius=0, color=(255, 255, 100), thickness=2)
        # iris
        for i in range(5):
            center = org_iris[i, 0:2].astype(np.int)
            aligned_facial_img_iris = cv2.circle(
                aligned_facial_img_iris, (center[0], center[1]), radius=0, color=(255, 0, 0), thickness=2)
        ...
        aligned_facial_img_iris = cv2.circle(aligned_facial_img_iris, (pupil_center[0], pupil_center[1]), radius=round(
            min(h_diameter, v_diameter)/2), color=(0, 0, 255), thickness=1)

    aligned_facial_img = np.hstack(
        [aligned_facial_img, aligned_facial_img_iris, aligned_facial_img_eyelids])
...
def extract_eye_features(img_path: str, face_landmarks, iris_marker):
    ...
    for _eye in resp:
        ...
        # eyelids distance
        eyelidsDistances[_eye] = eyelids_directed_hausdorff(
            set1_indices=[1, 8], set2_indices=[9, 16], landmarks=org_eye)
        # iris diameter
        irisesDiameters[_eye] = iris_diameter(org_iris)
        # distance from puipl center to the left corner (inner corner)
        pupil2corners[_eye] = np.linalg.norm(
            org_iris[0, 0:2] - org_eye[0, 0, 0:2])
...
if __name__ == '__main__':
    ...
    for video_path in videos_progress:
        ...
        input_path_file_hdf5 = os.path.normpath(
            os.path.join(input_path, "faceinfo.hdf5"))
        ...
        output_path = os.path.join(
            dataset_root, "eye_landmarks", args.dataset, video_name)
        output_file_path_csv = os.path.normpath(
            os.path.join(output_path, f"eyeinfo.csv"))
        output_file_path_hdf5 = os.path.normpath(
            os.path.join(output_path, f"eyeinfo.hdf5"))
        ...
        for index, row in tqdm.tqdm(_data_df.iterrows(), total=_data_df.shape[0], desc="frame"):
            ...
            if faces_not_found:
                ...
                facial_area = row[['left', 'top',
                                   'right', 'bottom']].astype(np.float)
                left_eye = row[['left_eye_x', 'left_eye_y']].astype(np.float)
                right_eye = row[['right_eye_x', 'right_eye_y']].astype(
                    np.float)
                ...
                eyelidsDistances, irisesDiameters, pupil2corners, meanColorCurve, stdColorCurve = extract_eye_features(
                    img_path, face_landmarks, iris_marker)
