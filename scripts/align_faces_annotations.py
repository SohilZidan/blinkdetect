...
import numpy as np
import tqdm
from bdlib.common import read_bbox_tag, read_bbox_rush
...
def extract_faces(annotations, all_detections):
    ...
    for _name in tqdm.tqdm(all_detections.keys(), total=len(all_detections.keys()), leave=False, desc="frame"):
        ...
        org_center = np.array(
            [(org_bbox[0]+0.5*org_bbox[2]), (org_bbox[1]+0.5*org_bbox[3])])
        #
        min_norm = 10000
        final_dets = {'face_1': {}}
        for face_id in dets:
            ...
            center_tmp = np.array(
                [(bbox_tmp[0]+bbox_tmp[2])/2, (bbox_tmp[1]+bbox_tmp[3])/2])
...
if __name__ == "__main__":
    ...
    for video_path in videos_progress:
        ...
        faceinfo_file_path_pkl = os.path.normpath(
            os.path.join(faces_root, "faceinfo_v2.pkl"))
        assert os.path.exists(
            faceinfo_file_path_pkl), f"faceinfo file is does not exist {faceinfo_file_path_pkl}"
        ...
        _new_detections = extract_faces(
            annotations=annotations, all_detections=all_detections)
